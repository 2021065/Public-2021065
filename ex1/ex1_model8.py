...
folder = [f for f in os.listdir(train_data_path) if os.path.isdir(os.path.join(train_data_path, f))] # ここを変更。データセット画像のフォルダ名（クラス名）を半角英数で入力
...
vqvae_trainer.fit(x_train_scaled, epochs=300, batch_size=64)
...
trained_vqvae_model.save("/home/takanolab/proglams_python/ex3/model/ex3_model1_{}".format(Z_DIM))
...
history = model2.fit(AnchorPositivePairs(num_batchs=32), epochs=1000)
model2.summary()
model2.save("/home/takanolab/proglams_python/ex3/model/ex3_model1M_{}".format(Z_DIM))
...
OUTPUT_DIR = "/home/takanolab/proglams_python/ex3/result/ex3_model1M_{}".format(Z_DIM)
...
for count2,i in enumerate(acc):
    ...
    for j in i:
        if j == 0:
            ...
            print('再現率label{}'.format(j),'=', acc[j]/y_test_count[i])
            print('適合率label{}'.format(j),'=', acc[j]/count[i])
        print('============')
...
for count, i in enumerate(acc):
  ...
  for count2, j in enumerate(i):
   if j == 0:
      ...
      print('再現率label',count2,'= {:.4f}'.format(j/y_test_count[count2]))
      print(j,'/',label_count[count2])
      print('適合率label',count2,'= {:.4f}'.format(j/count[i]))
