...
with connection:
    with connection.cursor() as cursor:
        # Create a new record
        sql = "UPDATE represent_data SET feature=%s WHERE id=%s"
        cursor.execute(sql, (str(df_mean0), '1'))
        sql = "UPDATE represent_data SET feature=%s WHERE id=%s"
        cursor.execute(sql, (str(df_mean1), '2'))
        sql = "UPDATE represent_data SET feature=%s WHERE id=%s"
        cursor.execute(sql, (str(df_mean2), '3'))
        sql = "UPDATE represent_data SET feature=%s WHERE id=%s"
        cursor.execute(sql, (str(df_mean3), '4'))
