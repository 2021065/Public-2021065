...
train_data_path = 'F:/国土地理院' # ここを変更。Colaboratoryにアップロードしたzipファイルを解凍後の、データセットのフォルダ名を入力
...
folder = ['org'] # ここを変更。データセット画像のフォルダ名（クラス名）を半角英数で入力
...
# 結果をクラスタごとにディレクトリに保存
for label, p in zip(predict_clus.labels_, x_test):
...
for i in range(pca_data.shape[0]):
    plt.scatter(pca_data[i,0], pca_data[i,1], c=color[int(predict_clus.labels_[i])])
